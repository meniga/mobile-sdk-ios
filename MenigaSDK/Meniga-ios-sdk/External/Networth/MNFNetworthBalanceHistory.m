...
-(MNFJob*)saveWithCompletion:(MNFErrorOnlyCompletionHandler)completion{
    ...
    __block MNFJob *job = [self updateWithApiPath:path pathQuery:nil jsonBody:jsonData httpMethod:kMNFHTTPMethodPUT service:MNFServiceNameNetWorth completion:^(MNFResponse * _Nullable response) {
...
-(MNFJob*)deleteWithCompletion:(MNFErrorOnlyCompletionHandler)completion{
    ...
    __block MNFJob *job = [self updateWithApiPath:path pathQuery:nil jsonBody:nil httpMethod:kMNFHTTPMethodDELETE service:MNFServiceNameNetWorth completion:^(MNFResponse * _Nullable response) {
        kObjectBlockDataDebugLog;
        [MNFObject executeOnMainThreadWithJob:job completion:completion error:response.error];
    }];
...
-(NSString*)description {
    return [NSString stringWithFormat:@"Networth balance history %@ identifier: %@, accountId: %@, balance: %@, balanceDate: %@, isDefault: %@",[super description],self.identifier,self.accountId,self.balance,self.balanceDate,self.isDefault];
}
