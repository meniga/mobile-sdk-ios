...
+ (void)initialize {
    NSData *apidata = [NSData dataWithContentsOfFile:[[NSBundle bundleForClass:self] pathForResource:@"apispec"
                                                                                              ofType:@"json"]];
    apiSpecDictionary = [NSJSONSerialization JSONObjectWithData:apidata options:0 error:nil];

    NSData *cashbackapidata = [NSData
        dataWithContentsOfFile:[[NSBundle bundleForClass:self] pathForResource:@"cashbackapispec" ofType:@"json"]];
...
+ (NSArray<NSDictionary *> *)filterParametersWithPath:(NSString *)path {
    ...
    for (NSDictionary *parameter in parameters) {
        if ([parameter[@"in"] isEqualToString:@"path"] || [parameter[@"name"] isEqualToString:@"skip"] ||
            [parameter[@"name"] isEqualToString:@"take"]) {
...
+ (id)randomValueForModel:(NSDictionary *)model {
    ...
    if ([modelFormat isEqualToString:@"date-time"]) {
        return [NSString stringWithFormat:@"2018-%02u-%02uT23:59:59.360Z",
                                          (arc4random_uniform(12) + 1),
                                          (arc4random_uniform(30) + 1)];
    }
