...
+ (void)loginWithUsername:(NSString *)theUsername
    ...
    NSData *postData = [NSJSONSerialization dataWithJSONObject:@{ @"email": theUsername, @"password": thePassword }
                                                       options:0
                                                         error:nil];
    ...
    NSURLSessionDataTask *task = [s_session
        dataTaskWithRequest:request
          completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
              s_cookies = [NSHTTPCookie cookiesWithResponseHeaderFields:[(NSHTTPURLResponse *)response allHeaderFields]
                                                                 forURL:response.URL];
              ...
              if (error != nil) {
                  ...
                  if ([statusCode intValue] >= 200 && [statusCode intValue] < 300) {
                      ...
                      if (completion != nil) {
                          completion(nil,
                                     [NSError errorWithDomain:@"Some domain"
                                                         code:-5000
                                                     userInfo:@{
                                                         NSLocalizedDescriptionKey: NSLocalizedString(
                                                             @"ServerUnreachable",
                                                             @"When we the email and pass is right and the status code "
                                                             @"we get is not related to authentication.")
                                                     }]);
                      }
                  }

              } else {
                  NSError *unexpectedError = [NSError
                      errorWithDomain:@""
                                 code:101
                             userInfo:@{
                                 NSLocalizedDescriptionKey: [NSString
                                     stringWithFormat:@"Unexpected response type. Expected class: %@. Got class: %@",
                                                      NSStringFromClass([NSHTTPURLResponse class]),
...
+ (NSURLRequest *)urlRequestWithURL:(NSURL *)theUrl
                         httpMethod:(NSString *)theHTTPMethod
                        httpHeaders:(NSDictionary *)theHttpHeaders
                         parameters:(NSData *)postData {
