...
+ (void)fetchDemoProfileIdsWithCompletion:(void (^)(NSArray<NSDictionary *> *, NSError *))completion {
    ...
    [[[self p_session] dataTaskWithRequest:request
...
+ (void)createDemoUserWithEmail:(NSString *)theEmail
...
+ (void)createRandomUserAndLoginWithCompletion:(void (^)(NSError *))completion {
    ...
    [self createRandomDemoUserWithCompletion:^(NSError *error) {
        if (error == nil) {
            [MENIGAAuthentication loginWithUsername:s_email
                                           password:s_password
                                     withCompletion:^(NSDictionary *tokenDictionary, NSError *error) {
                                         if (error == nil) {
...
+ (void)deleteUserWithCompletion:(nullable MNFErrorOnlyCompletionHandler)completion {
    ...
    [self apiRequestWithPath:kMNFApiPathUser
                   pathQuery:nil
                    jsonBody:nil
...
+ (void)startSynchronizationWithWaitTime:(NSNumber *)theWaitTime
                              completion:(void (^)(MNFSynchronization *_Nullable, NSError *_Nonnull))completion {
    [completion copy];
    ...
    [MNFObject apiRequestWithPath:kMNFApiPathSynchronization
                        pathQuery:nil
                         jsonBody:data
                       ...
                       completion:^(MNFResponse *response) {
                           if (response.error == nil) {
                               if ([response.result isKindOfClass:[NSDictionary class]]) {
                                   ...
                                   completion(nil,
                                              [MNFErrorUtils errorForUnexpectedDataOfType:[response.result class]
