...
- (void)testFetchFeed {
    ...
    MNFJob *job = [MNFFeed fetchFromDate:fromDate
                                  toDate:toDate
                                    skip:nil
                                    take:nil
...
- (void)testAppendDays {
    ...
    MNFJob *job = [MNFFeed
        ...
        withCompletion:^(MNFFeed *_Nullable feed, NSError *_Nullable error) {
            ...
            MNFJob *secondJob = [feed appendDays:@30
                                  withCompletion:^(NSArray<MNFFeedItem *> *newItems, NSError *_Nullable error) {
                                      XCTAssertNil(error);
...
- (void)testRefreshFeed {
    ...
    [MNFFeed fetchFromDate:fromDate
                    toDate:toDate
                      skip:nil
                      take:nil
            withCompletion:^(MNFFeed *_Nullable feed, NSError *_Nullable error) {
                ...
                MNFJob *job = [feed refreshFromServerFromDate:feed.from
                                                       toDate:feed.to
                                               withCompletion:^(NSArray<MNFFeedItem *> *newItems,
...
- (void)testFetchFeedTypes {
    ...
    MNFJob *job =
        [MNFFeed fetchFeedTypesWithCompletion:^(NSArray<NSString *> *_Nullable feedTypes, NSError *_Nullable error) {
            XCTAssertNil(error);
...
- (void)testFeedErrorData {
    ...
    [MNFFeed fetchFromDate:[NSDate date]
                    toDate:[NSDate date]
                      skip:@"jækladfajsæ"
                      take:nil
