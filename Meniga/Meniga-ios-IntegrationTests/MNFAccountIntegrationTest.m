...
- (void)testFetchAccounts {
    ...
    [MNFAccount fetchAccountsWithFilter:[[MNFAccountFilter alloc] init]
                             completion:^(NSArray *accounts, NSError *error){

                             }];

    MNFJob *job =
        [MNFAccount fetchAccountsWithFilter:nil
...
- (void)testFetchAccountWithId {
    ...
    [MNFAccount fetchAccountsWithFilter:nil
                             completion:^(NSArray<MNFAccount *> *_Nullable accounts, NSError *_Nullable error) {
                                 MNFJob *job = [MNFAccount
                                     fetchWithId:[accounts firstObject].identifier
                                      completion:^(MNFAccount *_Nullable account, NSError *_Nullable error) {
...
- (void)testSaveAccount {
    ...
    [MNFAccount fetchAccountsWithFilter:nil
                             completion:^(NSArray<MNFAccount *> *accounts, NSError *error) {
                         ...
                         MNFJob *job = [account
                             setMetadataValue:@"metadataValue"
                                   ...
                                   completion:^(NSError *_Nullable error) {
                                       ...
                                       MNFJob *secondJob =
                                           [account fetchMetadataForKey:@"metadataKey"
                                                         withCompletion:^(NSString *_Nullable metadataValue,
                                                                          NSError *_Nullable error) {
                                                             XCTAssertNil(error);
                                                             XCTAssertEqualObjects(metadataValue, @"metadataValue");
                                                             XCTAssertNotNil(metadataValue);

                                                             [account fetchMetadataWithCompletion:^(
                                                                          NSArray<NSDictionary *> *_Nullable metadatas,
                                                                          NSError *_Nullable error) {
                                                                 XCTAssertNil(error);
    ...
    [MNFAccount
        fetchAccountsWithFilter:nil
                     completion:^(NSArray<MNFAccount *> *_Nullable accounts, NSError *_Nullable error) {
                         ...
                         MNFJob *job = [account
                             fetchHistoryFromDate:[NSDate dateWithTimeIntervalSinceNow:-30 * 24 * 60 * 60]
                                           toDate:[NSDate date]
                                           sortBy:nil
                                        ascending:YES
                                       completion:^(NSArray<MNFAccountHistoryEntry *> *_Nullable accountHistoryEntries,
                                                    NSError *_Nullable error) {
                                           XCTAssertNil(error);
                         ...
                         [job handleCompletion:^(NSArray<MNFAccountHistoryEntry *> *_Nullable accountHistoryEntries,
                                                 id metadata,
                                                 NSError *error) {
                             XCTAssertNil(error);
                             XCTAssertNotNil(accountHistoryEntries);
...
- (void)testFetchAccountTypes {
    ...
    MNFJob *job = [MNFAccount
        fetchAccountTypesWithCompletion:^(NSArray<MNFAccountType *> *_Nullable accountTypes, NSError *_Nullable error) {
...
- (void)testFetchAccountCategories {
    ...
    MNFJob *job = [MNFAccount fetchAccountCategoriesWithCompletion:^(
                                  NSArray<MNFAccountCategory *> *_Nullable accountTypes, NSError *_Nullable error) {
        XCTAssertNotNil(accountTypes);
        XCTAssertNil(error);
...
- (void)testFetchAccountAuthorizationTypes {
    ...
    MNFJob *job = [MNFAccount
        fetchAccountAuthorizationTypesWithCompletion:^(
