    ...
    if (urlComponents.path.length > 0) {
        path = [urlComponents.path
            stringByAppendingString:
                path]; //If there's a path included in the base url it is in the path component after init. We therefore need to concatenate the base path with the path/endpoint parameter.
    }
