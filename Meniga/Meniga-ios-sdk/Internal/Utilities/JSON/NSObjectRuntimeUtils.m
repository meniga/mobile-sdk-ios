...
+ (id)objectWithClass:(Class)theClass
           modelArray:(NSArray<MNFJsonAdapterKeyAndProperty *> *)theArray
...
+ (BOOL)updateModel:(NSObject *)model
          Witharray:(NSArray<MNFJsonAdapterKeyAndProperty *> *)theArray
              error:(NSError *__autoreleasing *)theError {
    BOOL aggregatedSuccess = YES;

    for (MNFJsonAdapterKeyAndProperty *jsonKeyAndPropObj in theArray) {
        BOOL localSucess = [self validateAndSetValue:jsonKeyAndPropObj.propertyValue
                                         propertyKey:jsonKeyAndPropObj.propertyKey
                                             onModel:model
                                               error:theError];
        if (localSucess == NO) {
...
+ (BOOL)validateAndSetValue:(id)theValue
                propertyKey:(NSString *)thePropertyKey
                    onModel:(NSObject *)model
