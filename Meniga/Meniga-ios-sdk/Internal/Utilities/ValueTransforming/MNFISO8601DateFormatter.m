...
#endif
const unichar MNFISO8601DefaultTimeSeparatorCharacter = DEFAULT_TIME_SEPARATOR;
...
@interface MNFISO8601DateFormatter (UnparsingPrivate)
...
@implementation MNFISO8601DateFormatter
...
- (id)init {
    if ((self = [super init])) {
        ...
        timeSeparator = MNFISO8601DefaultTimeSeparatorCharacter;
        includeTime = NO;
        parsesStrictly = NO;
...
- (NSDateComponents *)dateComponentsFromString:(NSString *)string
    ...
    if ([string rangeOfString:@"/"].location != NSNotFound) return nil;
    ...
    NSUInteger
        ...
        month_or_week = NSDateComponentUndefined, day = NSDateComponentUndefined,
        //Time
        hour = NSDateComponentUndefined;
    NSTimeInterval minute = NAN, second = NAN;
    //Time zone
    NSInteger tz_hour = 0;
    ...
    if (strict) timeSep = MNFISO8601DefaultTimeSeparatorCharacter;
                            ...
                            if (!isdigit(*ch)) {
                                ...
                                switch (num_digits) {
                                    ...
                                    case 3: //DDD
                                        ...
                                        if (strict && (day > (365U + is_leap_year(year)))) isValidDate = NO;
                                        break;
        ...
        if (isValidDate) {
            ...
            switch (dateSpecification) {
                ...
                case week:;
                    ...
                    components.day = ((8U - Jan1Weekday) + (7U * (Jan1Weekday > thursday))) + (day - 1U)
                                     + (7U * (month_or_week - 2));
...
- (NSDate *)dateFromString:(NSString *)string timeZone:(out NSTimeZone **)outTimeZone range:(out NSRange *)outRange {
    ...
    if (outTimeZone) *outTimeZone = timeZone;
    if (components == nil) return nil;

    if (!timeZone) timeZone = self.defaultTimeZone;

    parsingCalendar.timeZone = timeZone;
    if ([components respondsToSelector:@selector(setTimeZone:)]) components.timeZone = timeZone;
    if ([components respondsToSelector:@selector(setCalendar:)]) components.calendar = parsingCalendar;
    ...
    switch (self.format) {
        ...
        default:
            [NSException raise:NSInternalInconsistencyException
                        format:@"self.format was %tu, not calendar (%tu), week (%tu), or ordinal (%tu)",
                               self.format,
...
- (NSString *)stringFromDate:(NSDate *)date formatString:(NSString *)dateFormat timeZone:(NSTimeZone *)timeZone {
    if (includeTime) {
        ...
        dateFormat = [dateFormat stringByAppendingFormat:@"'T'%@",
                                                         [self replaceColonsInString:timeFormat
...
- (NSString *)weekDateStringForDate:(NSDate *)date timeZone:(NSTimeZone *)timeZone {
    ...
    NSString *string = [NSString stringWithFormat:@"%lu-W%02lu-%02lu",
                                                  (unsigned long)year,
                                                  (unsigned long)week,
    ...
    if (includeTime) {
        ...
        unichar timeSep = self.timeSeparator;
        if (!timeSep) timeSep = MNFISO8601DefaultTimeSeparatorCharacter;
