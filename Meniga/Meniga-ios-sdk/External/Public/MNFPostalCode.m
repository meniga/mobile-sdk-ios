    ...
    __block MNFJob *job = [self apiRequestWithPath:path pathQuery:nil jsonBody:nil HTTPMethod:kMNFHTTPMethodGET service:MNFServiceNamePublic completion:^(MNFResponse * _Nullable response) {
        ...
        if (response.error == nil) {
            if ([response.result isKindOfClass:[NSDictionary class]]) {
                ...
                [MNFObject executeOnMainThreadWithJob:job completion:completion parameter:postalCode error:nil];
...
+(MNFJob*)fetchPostalCodesWithCompletion:(MNFMultiplePostalCodesCompletionHandler)completion {
    ...
    __block MNFJob *job = [self apiRequestWithPath:kMNFPublicPostalCodes pathQuery:nil jsonBody:nil HTTPMethod:kMNFHTTPMethodGET service:MNFServiceNamePublic completion:^(MNFResponse * _Nullable response) {
        ...
        if (response.error == nil) {
            if ([response.result isKindOfClass:[NSArray class]]) {
                ...
                [MNFObject executeOnMainThreadWithJob:job completion:completion parameter:postalCodes error:nil];
