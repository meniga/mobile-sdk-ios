...
+(MNFJob*)fetchPublicSettingsWithCompletion:(MNFPublicCompletionHandler)completion {
    ...
    __block MNFJob *job = [self apiRequestWithPath:kMNFPublicSettings pathQuery:nil jsonBody:nil HTTPMethod:kMNFHTTPMethodGET service:MNFServiceNamePublic completion:^(MNFResponse * _Nullable response) {
        ...
        if (response.error == nil) {
            if ([response.result isKindOfClass:[NSDictionary class]]) {
                ...
                [MNFObject executeOnMainThreadWithJob:job completion:completion parameter:publicSettings error:nil];
...
-(NSDictionary*)subclassedProperties {
    return @{
             @"currencies": [MNFJsonAdapterSubclassedProperty subclassedPropertyWithClass: [MNFCurrency class] option: kMNFAdapterOptionNoOption]
             };
}
