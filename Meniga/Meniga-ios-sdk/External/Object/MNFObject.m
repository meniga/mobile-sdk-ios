...
+ (nullable instancetype)initWithServerResult:(NSDictionary *)dictionary {
    if (dictionary != nil && [dictionary isKindOfClass:[NSDictionary class]] == YES && dictionary.allKeys.count != 0) {
        return [MNFJsonAdapter objectOfClass:[self class]
                                    jsonDict:dictionary
...
+ (MNFJob *)apiRequestWithPath:(NSString *)path
                     pathQuery:(nullable NSDictionary *)pathQuery
                      jsonBody:(nullable NSData *)postData
                    HTTPMethod:(NSString *)httpMethod
                       service:(MNFServiceName)service
    ...
    NSMutableDictionary *mutableHeaders =
        [[NSMutableDictionary alloc] initWithDictionary:[self p_authenticationProviderForService:service]];
...
+ (MNFJob *)apiRequestWithPath:(NSString *)path
                     pathQuery:(NSDictionary *)pathQuery
                      jsonBody:(NSData *)postData
                    HTTPMethod:(NSString *)httpMethod
                       service:(MNFServiceName)service
              percentageEncode:(BOOL)percentageEncode
                    completion:(MNFCompletionHandler)completion {
    [completion copy];

    NSString *baseURL = [Meniga apiURLForService:service];

    NSURL *url = [MNFURLConstructor URLFromBaseUrl:baseURL
                                              path:path
                                         pathQuery:pathQuery
                                 percentageEncoded:percentageEncode];
...
+ (MNFJob *)resourceRequestWithPath:(NSString *)path
                          pathQuery:(nullable NSDictionary *)pathQuery
                           jsonBody:(nullable NSData *)postData
                         HTTPMethod:(NSString *)httpMethod
                            service:(MNFServiceName)service
    ...
    [[MNFNetwork sharedNetwork] sendDownloadRequest:request
...
+ (void)executeOnMainThreadWithCompletion:(void (^)(id, id))completion
                           withParameters:(nullable id)firstParameter
                                      and:(nullable id)secondParameter {
    [completion copy];
    if (completion != nil) {
        if ([NSThread isMainThread] == YES) {
            __block typeof(completion) subCompletion = completion;
...
+ (void)executeOnMainThreadWithCompletion:(void (^)(id, id, id))completion
                           withParameters:(id)firstParameter
                           andSecondParam:(id)secondParameter
                            andThirdParam:(id)thirdParameter {
    [completion copy];
...
+ (void)executeOnMainThreadWithJob:(MNFJob *)theJob
                        completion:(void (^)(id _Nonnull))completion
...
+ (void)executeOnMainThreadWithJob:(MNFJob *)theJob
                        completion:(void (^)(id _Nonnull))completion
                             error:(nullable NSError *)theError {
...
+ (void)executeOnMainThreadWithJob:(MNFJob *)theJob
                        completion:(void (^)(id _Nonnull, NSError *_Nullable))completion
                         parameter:(id)parameter
...
+ (void)executeOnMainThreadWithJob:(MNFJob *)theJob
                        completion:(nonnull void (^)(id _Nonnull, id _Nonnull, NSError *_Nullable))completion
                   resultParameter:(id)theResultParam
                      metaDataParm:(id)theMetaDataParam
                             error:(NSError *)theError {
    [theJob setResult:theResultParam metaData:theMetaDataParam error:theError];

    [self executeOnMainThreadWithCompletion:completion
                             withParameters:theResultParam
                             andSecondParam:theMetaDataParam
                              andThirdParam:theError];
}

#pragma mark - saving

- (MNFJob *)updateWithApiPath:(NSString *)path
                    pathQuery:(NSDictionary *)pathQuery
                     jsonBody:(NSData *)data
                   httpMethod:(NSString *)httpMethod
                      service:(MNFServiceName)service
                   completion:(MNFCompletionHandler)completion {
    [completion copy];
    ...
    if (self.isDeleted) {
        completion([MNFResponse responseWithData:nil
                                           error:[MNFErrorUtils errorForDeletedObject:self]
                                      statusCode:[MNFErrorUtils errorForDeletedObject:self].code
                                    headerFields:nil]);
        return [MNFJob jobWithError:[MNFErrorUtils errorForDeletedObject:self]];
    }

    MNFJob *job = [[self class] apiRequestWithPath:path
                                         pathQuery:pathQuery
                                          jsonBody:data
                                        HTTPMethod:httpMethod
                                           service:service
...
- (MNFJob *)deleteWithApiPath:(NSString *)path
                    pathQuery:(NSDictionary *)pathQuery
                     jsonBody:(NSData *)data
                      service:(MNFServiceName)service
                   completion:(MNFCompletionHandler)completion {
    ...
    if (self.isDeleted == YES) {
        [MNFObject
            executeOnMainThreadWithCompletion:completion
                                withParameter:[MNFResponse responseWithData:nil
                                                                      error:[MNFErrorUtils errorForDeletedObject:self]
                                                                 statusCode:-100
                                                               headerFields:nil]];
    ...
    if (_mutableProperties == nil) {
        ...
        for (int i = 0; i < outCount; i++) {
            ...
            if (strstr(attr, "R") == NULL) { // Not read only
                NSString *propName = [NSString stringWithCString:property_getName(property)
            ...
            setValue:value
              forKey:
                  [NSString
                      stringWithFormat:
                          @"_%@",
                          key]]; //Setting the value of the synthezised ivar to avoid executing the custom setters of the mutable properties (and thereby updating the state).
    } else {
...
- (void)observeValueForKeyPath:(NSString *)keyPath
    ...
    MNFLogInfo(@"Changed mutable object property: %@ newValue:%@ oldValue:%@",
               keyPath,
               [change objectForKey:NSKeyValueChangeNewKey],
               [change objectForKey:NSKeyValueChangeOldKey]);
    MNFLogDebug(@"Changed mutable object property: %@ newValue:%@ oldValue:%@",
                keyPath,
                [change objectForKey:NSKeyValueChangeNewKey],
                [change objectForKey:NSKeyValueChangeOldKey]);
...
- (void)p_observeValueForKeyPath:(NSString *)keyPath
                 ofDeletedObject:(id)object
                          change:(NSDictionary<NSString *, id> *)change {
...
- (NSSet *)propertiesToIgnoreJsonSerialization {
    return [NSSet
        setWithObjects:@"objectstate", @"mutableProperties", @"keyValueStore", @"deleted", @"isNew", @"dirty", nil];
}
