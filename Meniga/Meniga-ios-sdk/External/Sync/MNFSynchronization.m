...
+ (MNFJob *)synchronizeWithTimeout:(NSNumber *)timeout
    ...
    [startJob handleCompletion:^(id _Nullable result, id _Nullable metaData, NSError *_Nullable error) {
        if (error != nil) {
            ...
            [self p_querySynchronizationWithSync:synchronization
                                           start:syncStart
                                         timeout:timeout
                                        interval:interval
...
+ (void)p_querySynchronizationWithSync:(MNFSynchronization *)sync
                                 start:(NSDate *)syncStart
                               timeout:(NSNumber *)timeout
                              interval:(NSNumber *)interval
    ...
    if (fabs(timeoutInterval) > [timeout doubleValue] / 1000 && ![sync.isSyncDone boolValue]) {
        NSError *timeoutError =
            [MNFErrorUtils errorWithCode:kMNFErrorMethodExecution
                                 message:@"Synchronization timed out before synchronization was completed"];
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, ([interval doubleValue] / 1000) * NSEC_PER_SEC),
                       ...
                       [syncJob
                           handleCompletion:^(id _Nullable result, id _Nullable metaData, NSError *_Nullable error) {
                               if (error != nil) {
                                   ...
                                   [self p_querySynchronizationWithSync:sync
                                                                  start:syncStart
                                                                timeout:timeout
                                                               interval:interval
                                                             completion:completion];
...
+ (MNFJob *)startSynchronizationWithWaitTime:(NSNumber *)waitTime
    ...
    NSData *data = [NSJSONSerialization dataWithJSONObject:@{ @"waitForCompleteMilliseconds": waitTime }
                                                   options:0
                                                     error:nil];

    __block MNFJob *job = [self
        apiRequestWithPath:kMNFApiPathSynchronization
                 pathQuery:nil
                  jsonBody:data
                HTTPMethod:kMNFHTTPMethodPOST
                   service:MNFServiceNameSync
...
+ (MNFJob *)startSynchronizationForRealmUserWithId:(NSNumber *)realmUserId
    ...
    NSMutableDictionary<NSString *, NSObject *> *dataDictionary =
        [@{ @"waitForCompleteMilliseconds": waitTime } mutableCopy];
    if (sessionToken != nil) {
...
- (BOOL)isSynchronizationNeeded {
    if (self.isSyncNeeded == nil) {
        MNFLogError(@"Unable to discern whether synchronization is needed. Please get current synchronization status "
                    @"to find out.");
        MNFLogInfo(@"Unable to discern whether synchronization is needed. Please get current synchronization status to "
                   @"find out.");
        MNFLogDebug(@"Unable to discern whether synchronization is needed. Please get current synchronization status "
                    @"to find out.");
        return NO;
    } else {
...
+ (MNFJob *)fetchRealmAuthenticationChallengeWithRealmId:(NSNumber *)realmId
                                              completion:(MNFSyncAuthenticationCompletionHandler)completion {
    [completion copy];

    NSString *path = [NSString stringWithFormat:@"%@/%@/auth", kMNFSynchronizationRealm, [realmId stringValue]];

    NSData *jsonData = [NSJSONSerialization
        dataWithJSONObject:@{ @"parameters": @[], @"saveDetails": @"true", @"realmUserIdentifier": @"" }
                   options:0
                     error:nil];
    ...
    __block MNFJob *job = [self
        apiRequestWithPath:path
                ...
                completion:^(MNFResponse *_Nullable response) {
                    ...
                    if (response.error == nil) {
                        if ([response.result isKindOfClass:[NSDictionary class]]) {
                            MNFSyncAuthenticationChallenge *authChallenge =
                                [MNFSyncAuthenticationChallenge initWithServerResult:response.result];

                            [MNFObject executeOnMainThreadWithJob:job
                                                       completion:completion
                                                        parameter:authChallenge
                                                            error:nil];
...
+ (MNFJob *)authenticateToRealmWithId:(NSNumber *)realmId
                       withParameters:(NSArray *)parameters
                         sessionToken:(NSString *)sessionToken
                          saveDetails:(NSNumber *)saveDetails
                  realmUserIdentifier:(NSString *)realmUserIdentifier
                           completion:(MNFSyncAuthenticationCompletionHandler)completion {
    [completion copy];
...
+ (MNFJob *)fetchAvailableRealmAccountsWithRealmUserId:(NSNumber *)realmUserId
                                          sessionToken:(NSString *)sessionToken
                                            completion:(MNFMultipleRealmAccountCompletionHandler)completion {
    [completion copy];

    NSString *path = [NSString stringWithFormat:@"%@/%@?sessionToken=%@",
                                                kMNFSynchronizationAccounts,
                                                [realmUserId stringValue],
                                                sessionToken];
...
+ (MNFJob *)authorizeRealmAccounts:(NSArray<MNFRealmAccount *> *)realmAccounts
    ...
    if (realmAccounts.count == 0) {
        NSError *error =
            [MNFErrorUtils errorWithCode:kMNFErrorInvalidOperation
                                 message:@"There needs to be at least 1 realmAccount to authenticate accounts."];
        [MNFObject executeOnMainThreadWithCompletion:completion withParameter:error];

        return [MNFJob jobWithError:error];
    }

    NSString *path = [NSString stringWithFormat:@"%@/%@/authorize?sessionToken=%@",
                                                kMNFSynchronizationAccounts,
                                                [realmUserId stringValue],
                                                sessionToken];
    ...
    __block MNFJob *job = [self apiRequestWithPath:path
                                         pathQuery:nil
                                          jsonBody:jsonData
                                        HTTPMethod:kMNFHTTPMethodPOST
                                           service:MNFServiceNameSync
...
- (NSString *)description {
    return [NSString
        stringWithFormat:
            @"Synchronization %@ syncHistoryId: %@, isSyncDone: %@, syncSessionStartTime: %@, realmSyncResponses: %@",
            [super description],
            self.syncHistoryId,
            self.isSyncDone,
            self.syncSessionStartTime,
            self.realmSyncResponses];
}
...
- (NSDictionary *)subclassedProperties {
    return @{
        @"realmSyncResponses":
            [MNFJsonAdapterSubclassedProperty subclassedPropertyWithClass:[MNFRealmSyncResponse class]
                                                                   option:kMNFAdapterOptionNoOption]
    };
}
