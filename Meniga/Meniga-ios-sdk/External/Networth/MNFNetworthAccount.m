...
+ (MNFJob *)fetchWithStartDate:(NSDate *)startDate
                 ...
                 interPolation:(BOOL)useInterpolation
                    completion:(MNFMultipleNetworthAccountsCompletionHandler)completion {
    return [self fetchWithStartDate:startDate
                            endDate:endDate
...
+ (MNFJob *)fetchWithStartDate:(NSDate *)startDate
    ...
    __block MNFJob *job = [self
        apiRequestWithPath:kMNFAPIPathNetworth
                ...
                completion:^(MNFResponse *_Nullable response) {
                    ...
                    if (response.error == nil) {
                        if ([response.result isKindOfClass:[NSArray class]]) {
                            NSArray *resultArray = [MNFJsonAdapter objectsOfClass:[self class]
                ...
                completion:^(MNFResponse *_Nullable response) {
                    ...
                    if (response.error == nil) {
                        if ([response.result isKindOfClass:[NSDictionary class]]) {
                            [MNFObject executeOnMainThreadWithJob:job
                                                       completion:completion
                                                        parameter:[self initWithServerResult:response.result]
                                                            error:nil];
...
+ (MNFJob *)firstEntrydateWithExcludedAccounts:(BOOL)excludedAccounts
                                    completion:(MNFSingleNetworthBalanceHistoryCompletionHandler)completion {
    MNFNumberToBoolValueTransformer *transformer = [MNFNumberToBoolValueTransformer transformer];

    NSDictionary *queryDict =
        @{ @"excludeAccountsExcludedFromNetWorth": [transformer reverseTransformedValue:@(excludedAccounts)] };

    __block MNFJob *job = [self
        apiRequestWithPath:kMNFNetworthFirstEntryDate
                 pathQuery:queryDict
                  jsonBody:nil
                HTTPMethod:kMNFHTTPMethodGET
                   service:MNFServiceNameNetWorth
                completion:^(MNFResponse *_Nullable response) {
                    ...
                    if (response.error == nil) {
                        if ([response.result isKindOfClass:[NSDictionary class]]) {
                            MNFNetworthBalanceHistory *firstHistory =
                                [MNFJsonAdapter objectOfClass:[MNFNetworthBalanceHistory class]
                                                     jsonDict:response.result
                                                       option:0
                                                        error:nil];

                            [MNFObject executeOnMainThreadWithJob:job
                                                       completion:completion
                                                        parameter:firstHistory
                                                            error:nil];
...
+ (MNFJob *)fetchNetworthTypesWithCompletion:(MNFMultipleAccountTypesCompletionHandler)completion {
    __block MNFJob *job = [self
        apiRequestWithPath:kMNFNetworthAccountTypes
                 pathQuery:nil
                  jsonBody:nil
                HTTPMethod:kMNFHTTPMethodGET
                   service:MNFServiceNameNetWorth
                completion:^(MNFResponse *_Nullable response) {
                    ...
                    if (response.error == nil) {
                        if ([response.result isKindOfClass:[NSArray class]]) {
                            NSArray *returnArray = [MNFJsonAdapter objectsOfClass:[MNFAccountType class]
                                                                        jsonArray:response.result
                                                                           option:kMNFAdapterOptionNoOption
                                                                            error:nil];
                            [MNFObject executeOnMainThreadWithJob:job
...
- (MNFJob *)saveWithCompletion:(MNFErrorOnlyCompletionHandler)completion {
    if (![self.isManual boolValue]) {
        NSError *error = [MNFErrorUtils errorWithCode:kMNFErrorInvalidOperation
                                              message:@"Only manually created networth accounts can be updated"];
    ...
    NSData *jsonBody =
        [MNFJsonAdapter JSONDataFromDictionary:@{ @"isExcluded": self.isExcluded, @"accountName": self.accountName }];
...
+ (MNFJob *)createWithInitialBalance:(NSNumber *)initialBalance
                             balance:(NSNumber *)balance
                   accountIdentifier:(NSString *)accountIdentifier
                         displayName:(NSString *)displayName
                        networthType:(NSString *)networthType
                  initialBalanceDate:(NSDate *)initialBalanceDate
                          completion:(MNFSingleNetworthAccountsCompletionHandler)completion {
    [completion copy];

    NSData *jsonData = [MNFJsonAdapter JSONDataFromDictionary:@{
        @"initialBalance": initialBalance,
        @"balance": balance,
        @"accountIdentifier": accountIdentifier,
        @"displayName": displayName,
        @"netWorthType": networthType,
        @"initialBalanceDate": [[NSDateUtils dateFormatter] stringFromDate:initialBalanceDate]
    }];

    __block MNFJob *job = [self
        apiRequestWithPath:kMNFNetworthAccounts
                 pathQuery:nil
                  jsonBody:jsonData
...
- (MNFJob *)deleteAccountWithCompletion:(MNFErrorOnlyCompletionHandler)completion {
    if (![self.isManual boolValue]) {
        MNFJob *errorJob = [MNFJob
            jobWithError:[MNFErrorUtils errorWithCode:kMNFErrorInvalidOperation
                                              message:@"Only manually created networth accounts can be deleted"]];
        [MNFObject
...
- (MNFJob *)addBalanceHistory:(MNFNetworthBalanceHistory *)balanceHistory
    ...
    NSString *path = [NSString
        stringWithFormat:@"%@/%@%@", kMNFNetworthAccounts, self.accountId, kMNFNetworthBalanceHistoryExtension];
    ...
    __block MNFJob *job = [self
        updateWithApiPath:path
               ...
               completion:^(MNFResponse *_Nullable response) {
                   ...
                   if (response.error == nil) {
                       if ([response.result isKindOfClass:[NSDictionary class]]) {
                           [MNFJsonAdapter refreshObject:balanceHistory
                                            withJsonDict:response.result
                                                  option:kMNFAdapterOptionNoOption
                                                   error:nil];
                       } else {
                       ...
                       if ([self.history
                               filteredArrayUsingPredicate:[NSPredicate predicateWithFormat:@"identifier == %@",
                                                                                            balanceHistory.identifier]]
                               .count
                           == 0) {
...
- (NSString *)description {
    return [NSString
        stringWithFormat:
            @"Networth account %@ accountId: %@, realmAccountTypeId: %@, accountName: %@, isImport: %@, isManual: %@, "
            @"isExcluded: %@, netWorthType: %@, currentBalance: %@, history: %@, accountType: %@",
            [super description],
            self.accountId,
            self.accountTypeId,
            self.accountName,
...
- (NSDictionary<NSString *, MNFJsonAdapterSubclassedProperty *> *)subclassedProperties {
    return @{
        @"history": [MNFJsonAdapterSubclassedProperty subclassedPropertyWithClass:[MNFNetworthBalanceHistory class]
                                                                           option:kMNFAdapterOptionNoOption],
        @"accountType": [MNFJsonAdapterSubclassedProperty subclassedPropertyWithClass:[MNFAccountType class]
                                                                               option:kMNFAdapterOptionNoOption],
        @"accountTypeCategory": [MNFJsonAdapterSubclassedProperty subclassedPropertyWithClass:[MNFAccountCategory class]
                                                                                       option:kMNFAdapterOptionNoOption]
    };
}
