...
+(MNFJob *)fetchTermsAndConditionsWithCompletion:(MNFMultipleTermsAndConditionsCompletionHandler)completion {
    ...
    __block MNFJob *job = [MNFObject apiRequestWithPath: kMNFApiPathTerms pathQuery:nil jsonBody:nil HTTPMethod:kMNFHTTPMethodGET service:MNFServiceNameTerms completion: ^(MNFResponse *response) {
        ...
        if (response.error == nil) {
            ...
            if ([response.result isKindOfClass: [NSArray class]]) {
                ...
                [MNFObject executeOnMainThreadWithJob: job completion: completion parameter: termsAndConditions error: nil];
...
+(MNFJob *)fetchTermsAndConditionsWithId:(NSNumber *)identifier completion:(MNFTermsAndConditionsCompletionHandler)completion {
    ...
    __block MNFJob *job = [self apiRequestWithPath: path pathQuery: nil jsonBody: nil HTTPMethod: kMNFHTTPMethodGET service:MNFServiceNameTerms completion:^(MNFResponse *response) {
        
        if (response.error == nil) {
            
            if ([response.result isKindOfClass: [NSDictionary class]]) {
                
                MNFTermsAndConditions *termsAndConditions = [MNFTermsAndConditions initWithServerResult: response.result];
...
+(MNFJob *)acceptTermsAndConditionsWithId:(NSNumber*)identifier completion:(MNFErrorOnlyCompletionHandler)completion{
    ...
    __block MNFJob *job = [MNFObject apiRequestWithPath: path pathQuery:nil jsonBody:nil HTTPMethod:kMNFHTTPMethodPOST service:MNFServiceNameTerms completion: ^(MNFResponse *response) {
        
        [MNFObject executeOnMainThreadWithJob: job completion: completion parameter: response.error];
...
-(NSDictionary <NSString *, MNFJsonAdapterSubclassedProperty *> *)subclassedProperties {
    ...
    return @{
             @"termsAndConditionsType" : [MNFJsonAdapterSubclassedProperty subclassedPropertyWithClass: [MNFTermsAndConditionType class] delegate: [[MNFTermsAndConditionType alloc] init] option: kMNFAdapterOptionNoOption]
             };
}
