...
+ (MNFJob *)authenticateWithEmail:(NSString *)email
                       ...
                       completion:(MNFAuthenticationCompletionHandler)completion {
...
+ (MNFJob *)authenticateWithEmail:(NSString *)email
    ...
    __block MNFJob *job = [self
        apiRequestWithPath:kMNFApiPathAuthentication
                 pathQuery:nil
                  jsonBody:jsonData
                HTTPMethod:kMNFHTTPMethodPOST
                   service:MNFServiceNameAuthentication
                completion:^(MNFResponse *_Nullable response) {
                    kObjectBlockDataDebugLog;

                    if (response.error == nil) {
                        if ([response.result isKindOfClass:[NSDictionary class]]) {
                            MNFAuthentication *authentication =
                                [MNFAuthentication initWithServerResult:response.result];
...
+ (MNFJob *)authenticateAfterWithUserIdentifier:(NSString *)userIdentifier
    ...
    __block MNFJob *job = [self apiRequestWithPath:kMNFAuthenticationAfter
                                         pathQuery:nil
                                          jsonBody:jsonData
                                        HTTPMethod:kMNFHTTPMethodPOST
                                           service:MNFServiceNameAuthentication
...
+ (MNFJob *)authenticateWithSSOToken:(NSString *)token
                      ...
                      withCompletion:(MNFAuthenticationCompletionHandler)completion {
    [completion copy];
...
+ (MNFJob *)refreshTokensWithRefreshToken:(NSString *)refreshToken
                               completion:(MNFAuthenticationCompletionHandler)completion {
    return [self refreshTokensWithRefreshToken:refreshToken
                                      clientId:nil
                                  clientSecret:nil
                                       subject:nil
                                    completion:completion];
...
+ (MNFJob *)refreshTokensWithRefreshToken:(NSString *)refreshToken
    ...
    __block MNFJob *job = [self
        apiRequestWithPath:kMNFAuthenticationRefresh
                 pathQuery:nil
                  jsonBody:jsonData
                HTTPMethod:kMNFHTTPMethodPOST
                   service:MNFServiceNameAuthentication
