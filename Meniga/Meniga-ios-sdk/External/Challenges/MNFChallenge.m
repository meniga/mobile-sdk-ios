    ...
    __block MNFJob *job= [self apiRequestWithPath:path pathQuery:nil jsonBody:nil HTTPMethod:kMNFHTTPMethodGET service:MNFServiceNameChallenges completion:^(MNFResponse * _Nullable response) {
        ...
        if (response.error == nil) {
            if ([response.result isKindOfClass:[NSDictionary class]]) {
                ...
                [MNFObject executeOnMainThreadWithJob:job completion:completion parameter:challenge error:nil];
...
-(MNFJob*)saveWithCompletion:(MNFErrorOnlyCompletionHandler)completion {
    ...
    NSMutableDictionary *jsonDict = [[MNFJsonAdapter JSONDictFromObject:self option:kMNFAdapterOptionNoOption error:nil] mutableCopy];
    NSDictionary *modelDict = [MNFJsonAdapter JSONDictFromObject:self.challengeModel option:kMNFAdapterOptionNoOption error:nil];
    jsonDict[@"typeData"] = modelDict;
    NSData *jsonData = [NSJSONSerialization dataWithJSONObject:[jsonDict copy] options:0 error:nil];
    
    __block MNFJob *job = [self updateWithApiPath:path pathQuery:nil jsonBody:jsonData httpMethod:kMNFHTTPMethodPUT service:MNFServiceNameChallenges completion:^(MNFResponse * _Nullable response) {
...
-(MNFJob*)deleteWithCompletion:(MNFErrorOnlyCompletionHandler)completion {
    ...
    __block MNFJob *job = [self deleteWithApiPath:path pathQuery:nil jsonBody:nil service:MNFServiceNameChallenges completion:^(MNFResponse * _Nullable response) {
...
+(MNFJob*)challengeWithTitle:(NSString *)title
    ...
    __block MNFJob *job = [MNFObject apiRequestWithPath:kMNFApiPathChallenges pathQuery:nil jsonBody:jsonData HTTPMethod:kMNFHTTPMethodPOST service:MNFServiceNameChallenges completion:^(MNFResponse * _Nullable response) {
...
+ (MNFJob*)fetchIconIdentifiersWithFormat:(NSString *)format completion:(MNFChallengeIconIdentifiersCompletionHandler)completion {
    [completion copy];
    
    NSMutableDictionary *jsonQuery = [NSMutableDictionary dictionary];
    jsonQuery[@"format"] = format;
    
    __block MNFJob *job = [self apiRequestWithPath:kMNFChallengesIcons pathQuery:[jsonQuery copy] jsonBody:nil HTTPMethod:kMNFHTTPMethodGET service:MNFServiceNameChallenges completion:^(MNFResponse * _Nullable response) {
        ...
        if (response.error == nil) {
            if ([response.result isKindOfClass:[NSArray class]]) {
                ...
                [MNFObject executeOnMainThreadWithJob:job completion:completion parameter:[iconIdentifiers copy] error:nil];
            }
            else {
...
+ (MNFJob*)fetchIconWithIdentifier:(NSString *)iconIdentifier completion:(MNFChallengeIconImageDataCompletionHandler)completion {
    [completion copy];
...
-(NSSet *)propertiesToIgnoreJsonSerialization {
    return [NSSet setWithObjects:@"objectstate",@"dirty",@"deleted",@"isNew",@"mutableProperties",@"keyValueStore",@"identifier",@"type",@"accepted",@"acceptedDate",@"topicId",@"challengeModel", nil];
}
