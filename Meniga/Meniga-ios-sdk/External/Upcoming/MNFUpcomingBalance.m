...
+(MNFJob*)fetchBalancesWithDateTo:(NSDate *)dateTo includeOverdueFromDate:(NSDate *)overdueFromDate accountIds:(NSString *)accountIds includeUnlinked:(BOOL)includeUnlinked useAvailableAmount:(BOOL)useAvailableAmount completion:(MNFBalancesCompletionHandler)completion {
    [completion copy];
    ...
    __block MNFJob *job = [MNFObject apiRequestWithPath:KMNFUpcomingBalances pathQuery:[jsonQuery copy] jsonBody:nil HTTPMethod:kMNFHTTPMethodGET service:MNFServiceNameUpcoming completion:^(MNFResponse * _Nullable response) {
        ...
        if (response.error == nil) {
            if ([response.result isKindOfClass:[NSDictionary class]]) {
                ...
                [MNFObject executeOnMainThreadWithJob:job completion:completion parameter:balance error:nil];
...
-(NSDictionary*)subclassedProperties {
    return @{
             @"balanceDates":[MNFJsonAdapterSubclassedProperty subclassedPropertyWithClass:[MNFUpcomingBalanceDate class] option: kMNFAdapterOptionNoOption]
             };
}
