...
+(MNFJob*)fetchWithId:(NSNumber*)identifier completion :(MNFRecurringPatternCompletionHandler)completion {
    ...
    __block MNFJob *job = [self apiRequestWithPath:path pathQuery:nil jsonBody:nil HTTPMethod:kMNFHTTPMethodGET service:MNFServiceNameUpcoming completion:^(MNFResponse*  _Nullable response) {
        ...
        if (response.error == nil) {
            if ([response.result isKindOfClass:[NSDictionary class]]) {
                ...
                MNFUpcomingRecurringPattern *pattern = [MNFUpcomingRecurringPattern initWithServerResult:response.result];
                
                [MNFObject executeOnMainThreadWithJob:job completion:completion parameter:pattern error:nil];
...
+(MNFJob*)fetchRecurringPatternsWithStatuses:(NSString *)statuses types:(NSString *)types completion:(MNFMultipleRecurringPatternsCompletionHandler)completion {
    [completion copy];
    ...
    __block MNFJob *job = [MNFObject apiRequestWithPath:kMNFUpcomingRecurring pathQuery:[jsonQuery copy] jsonBody:nil HTTPMethod:kMNFHTTPMethodGET service:MNFServiceNameUpcoming completion:^(MNFResponse * _Nullable response) {
        kObjectBlockDataDebugLog;
        ...
        if (response.error == nil) {
            if ([response.result isKindOfClass:[NSArray class]]) {
                ...
                [MNFObject executeOnMainThreadWithJob:job completion:completion parameter:patterns error:nil];
...
-(NSDictionary*)subclassedProperties {
    return @{
             @"pattern":[MNFJsonAdapterSubclassedProperty subclassedPropertyWithClass:[MNFUpcomingPattern class] option: kMNFAdapterOptionNoOption]
             };
}
