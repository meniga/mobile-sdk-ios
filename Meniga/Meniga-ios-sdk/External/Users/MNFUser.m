...
+ (MNFJob *)registerUserWithEmail:(NSString *)email
                         password:(NSString *)password
                          culture:(NSString *)culture
                       completion:(MNFUserCompletionHandler)completion {
...
+ (MNFJob *)beginRegistrationWithEmail:(NSString *)email completion:(MNFErrorOnlyCompletionHandler)completion {
    ...
    __block MNFJob *job = [self apiRequestWithPath:kMNFUserBeginRegistration
                                         pathQuery:nil
                                          jsonBody:jsonData
...
+ (MNFJob *)registerUserWithEmail:(NSString *)email
    ...
    __block MNFJob *job = [self
        apiRequestWithPath:kMNFUserRegister
                 pathQuery:nil
                  jsonBody:jsonData
                HTTPMethod:kMNFHTTPMethodPOST
                   service:MNFServiceNameUsers
...
+ (MNFJob *)fetchUsersWithCompletion:(MNFMultipleUsersCompletionHandler)completion {
    ...
    __block MNFJob *job = [self
        apiRequestWithPath:kMNFApiPathUser
                 pathQuery:jsonQuery
                  jsonBody:nil
...
- (MNFJob *)saveWithCompletion:(nullable MNFErrorOnlyCompletionHandler)completion {
    ...
    __block MNFJob *job = [self updateWithApiPath:kMNFApiPathUser
                                        pathQuery:nil
                                         jsonBody:jsonData
...
- (MNFJob *)deleteUserWithCompletion:(nullable MNFErrorOnlyCompletionHandler)completion {
    ...
    __block MNFJob *job = [self deleteWithApiPath:kMNFApiPathUser
                                        pathQuery:nil
                                         jsonBody:nil
                                          service:MNFServiceNameUsers
                                       completion:^(MNFResponse *_Nullable response) {
...
- (MNFJob *)changeCulture:(NSString *)culture withCompletion:(nullable MNFErrorOnlyCompletionHandler)completion {
    ...
    __block MNFJob *job = [self updateWithApiPath:kMNFUserCulture
                                        pathQuery:jsonQuery
                                         jsonBody:nil
                                       httpMethod:kMNFHTTPMethodPUT
                                          service:MNFServiceNameUsers
...
- (MNFJob *)optInWithCompletion:(nullable MNFErrorOnlyCompletionHandler)completion {
    ...
    __block MNFJob *job = [[self class] apiRequestWithPath:kMNFUserOptin
                                                 pathQuery:nil
                                                  jsonBody:nil
                                                HTTPMethod:kMNFHTTPMethodPUT
                                                   service:MNFServiceNameUsers
...
- (MNFJob *)optOutWithCompletion:(nullable MNFErrorOnlyCompletionHandler)completion {
    ...
    __block MNFJob *job = [[self class] apiRequestWithPath:kMNFUserOptout
                                                 pathQuery:nil
                                                  jsonBody:nil
                                                HTTPMethod:kMNFHTTPMethodPUT
                                                   service:MNFServiceNameUsers
...
- (MNFJob *)fetchMetaDataForKeys:(NSString *)keys completion:(MNFMultipleMetadataCompletionHandlers)completion {
    ...
    __block MNFJob *job = [[self class] apiRequestWithPath:kMNFUserMetaData
                                                 pathQuery:jsonQuery
                                                  jsonBody:nil
                                                HTTPMethod:kMNFHTTPMethodGET
                                                   service:MNFServiceNameUsers
...
- (MNFJob *)updateMetaDataForKey:(NSString *)key
...
- (NSString *)description {
    return
        [NSString stringWithFormat:@"User %@ identifier: %@, personId: %@, firstName: %@, lastName: %@, lastLoginDate: "
                                   @"%@, isInitialSetupDone: %@, email: %@, culture: %@, lastLoginRemoteHost: %@, "
                                   @"createDate: %@, termsAndConditionsId: %@, termsAndConditionsAcceptDate: %@, "
                                   @"optOutDate: %@, profile: %@, phoneNumber: %@, passwordExpiryDate: %@, hide: %@",
                                   [super description],
                                   self.identifier,
                                   self.personId,
                                   self.firstName,
                                   self.lastName,
                                   self.lastLoginDate,
                                   self.isInitialSetupDone,
                                   self.email,
                                   self.culture,
                                   self.lastLoginRemoteHost,
                                   self.createDate,
                                   self.termsAndConditionsId,
                                   self.termsAndConditionsAcceptDate,
                                   self.optOutDate,
                                   self.profile,
                                   self.phoneNumber,
                                   self.passwordExpiryDate,
                                   self.hide];
}
...
- (NSDictionary *)propertyValueTransformers {
    return @{
        @"lastLoginDate": [MNFBasicDateValueTransformer transformer],
...
- (NSDictionary *)subclassedProperties {
    return @{
        @"profile": [MNFJsonAdapterSubclassedProperty subclassedPropertyWithClass:[MNFUserProfile class]
                                                                           option:kMNFAdapterOptionNoOption]
    };
}
