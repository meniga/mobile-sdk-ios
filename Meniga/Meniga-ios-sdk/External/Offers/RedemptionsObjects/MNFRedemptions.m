...
- (NSDictionary<NSString *, MNFJsonAdapterSubclassedProperty *> *)subclassedProperties {
    return @{
        @"scheduledReimbursements":
            [MNFJsonAdapterSubclassedProperty subclassedPropertyWithClass:[MNFScheduledReimbursement class]
...
+ (MNFJob *)fetchRedemptionsFromDate:(NSDate *)fromDate
                              toDate:(NSDate *)toDate
                                skip:(NSNumber *)skip
                                take:(NSNumber *)take
                          completion:(nonnull MNFRedemptionsCompletionHandler)completion {
                    ...
                    if (response.error == nil) {
                        if ([response.result isKindOfClass:[NSArray class]] == YES &&
                            ...
                            MNFRedemptionsMetaData *metaData =
                                [MNFRedemptionsMetaData initWithServerResult:response.metaData];

                            [MNFObject executeOnMainThreadWithJob:job
                                                       completion:completion
                                                  resultParameter:offerTransactions
