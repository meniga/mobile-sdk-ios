                ...
                completion:^(MNFResponse *response) {
                    ...
                    if (response.error == nil) {
                        if ([response.result isKindOfClass:[NSDictionary class]] == YES) {
                            ...
                            NSError *error = [MNFErrorUtils errorForUnexpectedDataOfType:[response.result class]
                                                                                expected:[NSDictionary class]];
...
+ (MNFJob *)fetchOffersWithFilter:(MNFOfferFilter *)offerFilter
                             take:(NSNumber *)take
                             skip:(NSNumber *)skip
                       completion:(MNFMultipleOffersCompletionHandler)completion {
    [completion copy];

    NSMutableDictionary *queryDict = [NSMutableDictionary dictionary];

    if (offerFilter != nil) {
        NSDictionary *filterDict = [MNFJsonAdapter JSONDictFromObject:(id<MNFJsonAdapterDelegate>)offerFilter
                                                               option:0
                                                                error:nil];
        if (filterDict != nil) {
                ...
                completion:^(MNFResponse *response) {
                    ...
                    if (response.error == nil) {
                        if ([response.result isKindOfClass:[NSArray class]] == YES) {
                            ...
                            [MNFObject executeOnMainThreadWithJob:job
                                                       completion:completion
                                                  resultParameter:offers
                                                     metaDataParm:response.metaData
                                                            error:response.error];
                            ...
                            [MNFObject
                                executeOnMainThreadWithJob:job
                                                completion:completion
                                           resultParameter:response.result
                                              metaDataParm:response.metaData
                                                     error:[MNFErrorUtils
                                                               errorForUnexpectedDataOfType:[response.result class]
                ...
                completion:^(MNFResponse *response) {
                    ...
                    if (response.error == nil) {
                        if ([response.result isKindOfClass:[NSArray class]]) {
                            NSArray<MNFMerchantLocation *> *merchantLocations =
                                [MNFJsonAdapter objectsOfClass:[MNFMerchantLocation class]
                                                     jsonArray:response.result
                                                        option:kMNFAdapterOptionNoOption
                                                         error:nil];
                            [MNFObject executeOnMainThreadWithJob:job
                                                       completion:completion
                                                        parameter:merchantLocations
                                                            error:nil];
                        } else {
                            [MNFObject
                                executeOnMainThreadWithJob:job
    ...
    __block MNFJob *job = [MNFObject
        apiRequestWithPath:path
                ...
                completion:^(MNFResponse *response) {
                    ...
                    if (response.error == nil) {
                        if ([response.result isKindOfClass:[NSArray class]] == YES &&
                            [response.metaData isKindOfClass:[NSDictionary class]] == YES) {
                            NSArray<MNFSimilarBrand *> *similarBrands =
                                [MNFSimilarBrand initWithServerResults:response.result];
                            MNFSimilarBrandMetaData *metaData =
                                [MNFSimilarBrandMetaData initWithServerResult:response.metaData];
                            [MNFObject executeOnMainThreadWithJob:job
                                                       completion:completion
                                                  resultParameter:similarBrands
                                                     metaDataParm:metaData
                                                            error:nil];
                                ...
                                [MNFObject
                                    executeOnMainThreadWithJob:job
                                               ...
                                               resultParameter:nil
                                                  metaDataParm:nil
                                                         error:[MNFErrorUtils
                                                                   errorForUnexpectedDataOfType:[response.metaData
                                                                                                    class]
                                                                                       expected:[NSDictionary class]]];
                            }
                        }
    ...
    __block MNFJob *job = [MNFObject
        apiRequestWithPath:path
                ...
                completion:^(MNFResponse *response) {
                    ...
                    if (response.error == nil) {
                        if ([response.result isKindOfClass:[NSArray class]] == YES) {
                            NSArray<MNFOfferTransaction *> *offerTransactions =
                                [MNFOfferTransaction initWithServerResults:response.result];
                            [MNFObject executeOnMainThreadWithJob:job
                                                       completion:completion
                                                        parameter:offerTransactions
                                                            error:nil];
                        } else {
                            [MNFObject
                                executeOnMainThreadWithJob:job
...
- (MNFJob *)refreshWithCompletion:(MNFErrorOnlyCompletionHandler)completion {
    ...
    MNFJob *job = [MNFOffer fetchWithId:self.identifier
                             completion:^(MNFOffer *offer, NSError *error) {
                                 if (error == nil) {
...
- (NSDictionary<NSString *, MNFJsonAdapterSubclassedProperty *> *)subclassedProperties {
    return @{
        @"relevanceHook": [MNFJsonAdapterSubclassedProperty subclassedPropertyWithClass:[MNFOfferRelevanceHook class]
                                                                                 option:kMNFAdapterOptionNoOption],
        @"merchantLocations": [MNFJsonAdapterSubclassedProperty subclassedPropertyWithClass:[MNFMerchantLocation class]
                                                                                     option:kMNFAdapterOptionNoOption]
    };
}
