...
+(nonnull MNFJob*)fetchMerchantSeriesWithTransactionFilter:(nonnull MNFTransactionFilter*)seriesFilter merchantSeriesOptions:(nonnull MNFMerchantSeriesOptions*)merchantOptions withCompletion:(nullable MNFMerchantSeriesCompletionHandler)completion{
    

    NSDictionary *jsonDict = @{@"transactionFilter":[MNFJsonAdapter JSONDictFromObject:seriesFilter option:0 error:nil], @"options":[MNFJsonAdapter JSONDictFromObject:merchantOptions option:0 error:nil]};
   ...
   __block MNFJob *job = [self apiRequestWithPath:path pathQuery:nil jsonBody:jsonData HTTPMethod:kMNFHTTPMethodPOST service:MNFServiceNameMerchants completion:^(MNFResponse * _Nullable response) {
        ...
        if (response.error == nil) {
            ...
            if ([response.result isKindOfClass:[NSArray class]]) {
                ...
                [MNFObject executeOnMainThreadWithJob:job completion:completion parameter:merchantSeries error:nil];
