...
+ (MNFJob*)fetchOrganizationsWithNameSearch:(NSString *)nameSearch completion:(MNFOrganizationsCompletionHandler)completion {
    [completion copy];
    
    NSMutableDictionary *pathQuery = [NSMutableDictionary dictionary];
    pathQuery[@"nameSearch"] = nameSearch;
    
    __block MNFJob *job = [self apiRequestWithPath:kMNFApiPathOrganizations pathQuery:[pathQuery copy] jsonBody:nil HTTPMethod:kMNFHTTPMethodGET service:MNFServiceNameOrganizations completion:^(MNFResponse*  _Nullable response) {
        ...
        if (response.error == nil) {
            if ([response.result isKindOfClass:[NSArray class]]) {
                ...
                [MNFObject executeOnMainThreadWithJob:job completion:completion parameter:organizations error:nil];
...
-(NSDictionary*)subclassedProperties {
    return @{
             @"realms": [MNFJsonAdapterSubclassedProperty subclassedPropertyWithClass: [MNFOrganizationRealm class] option: kMNFAdapterOptionNoOption]
             };
}
