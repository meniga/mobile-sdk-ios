...
+(MNFJob*)fetchTransactionSuggestionsWithText:(NSString *)text suggestionTypes:(NSString *)suggestionTypes onlyShowResultsWithTransactions:(NSNumber *)onlyShowResultsWithTransactions take:(NSNumber *)take sort:(NSString *)sort filter:(MNFTransactionFilter *)filter completion:(MNFTransactionSuggestionsCompletionHandler)completion {
    [completion copy];
    ...
    __block MNFJob *job = [self apiRequestWithPath:kMNFTransactionSuggestions pathQuery:[jsonQuery copy] jsonBody:nil HTTPMethod:kMNFHTTPMethodGET service:MNFServiceNameTransactions completion:^(MNFResponse * _Nullable response) {
        ...
        if (response.error == nil) {
            ...
            if ([response.result isKindOfClass:[NSArray class]]) {
                ...
                [MNFObject executeOnMainThreadWithJob:job completion:completion parameter:suggestions error:nil];
