...
+ (MNFJob *)fetchRulesWithCompletion:(MNFMultipleTransactionRulesCompleitonHandler)completion {
    ...
    __block MNFJob *job = [self
        apiRequestWithPath:kMNFTransactionsRule
                 pathQuery:nil
                  jsonBody:nil
                HTTPMethod:kMNFHTTPMethodGET
                   service:MNFServiceNameTransactions
...
+ (MNFJob *)createRuleWithName:(NSString *)ruleName
                  textCriteria:(NSString *)textCriteria
      textCriteriaOperatorType:(NSNumber *)operatorType
         dateMatchTypeCriteria:(NSNumber *)dateMatchTypeCriteria
             daysLimitCriteria:(NSNumber *)daysLimitCriteria
       amountLimitTypeCriteria:(NSNumber *)amountLimitTypeCriteria
       amountLimitSignCriteria:(NSNumber *)amountLimitSignCriteria
                amountCriteria:(NSNumber *)amountCritera
       accountCategoryCriteria:(NSString *)accountCategoryCriteria
                  acceptAction:(NSNumber *)acceptAction
              monthShiftAction:(NSNumber *)monthShiftAction
                  removeAction:(NSNumber *)removeAction
                    textAction:(NSString *)textAction
                 commentAction:(NSString *)commentAction
              categoryIdAction:(NSNumber *)categoryIdAction
                  splitActions:(NSArray *)splitActions
                    flagAction:(NSNumber *)flagAction
               applyOnExisting:(NSNumber *)applyOnExisting
                    completion:(MNFTransactionRuleCompletionHandler)completion {
    NSDictionary *queryDict = @{
        @"applyOnExisting": [[MNFNumberToBoolValueTransformer transformer] reverseTransformedValue:applyOnExisting]
    };
    ...
    if (splitActions != nil) {
        serializedSplitActions = [MNFJsonAdapter JSONArrayFromArray:splitActions
...
- (MNFJob *)saveAndApplyOnExisting:(NSNumber *)applyOnExisting completion:(MNFErrorOnlyCompletionHandler)completion {
    ...
    [jsonQuery setObject:[[MNFNumberToBoolValueTransformer transformer] reverseTransformedValue:applyOnExisting]
                  forKey:@"applyOnExisting"];

    NSData *jsonData = [MNFJsonAdapter JSONDataFromObject:self option:kMNFAdapterOptionNoOption error:nil];

    NSString *path = [kMNFTransactionsRule
        stringByAppendingString:[NSString stringWithFormat:@"/%@", [self.identifier stringValue]]];
...
- (NSString *)description {
    return [NSString
        stringWithFormat:
            @"Transaction rule %@ identifier: %@, userId: %@, name: %@, createdDate: %@, modifiedDate: %@, "
            @"textCriteria: %@, textCriteriaOperatorType: %@, dateMatchTypeCriteria: %@, daysLimitCriteria: %@, "
            @"amountLimitTypeCriteria: %@, amountLimitSignCriteria: %@, amountCriteria: %@, accountCategoryCriteria: "
            @"%@, acceptAction: %@, monthShiftAction: %@, removeAction: %@, textAction: %@, commentAction: %@, "
            @"tagAction: %@, categoryIdAction: %@, splitActions: %@, flagAction: %@",
            [super description],
            self.identifier,
            self.userId,
            self.name,
            ...
            self.modifiedDate,
            self.textCriteria,
            self.textCriteriaOperatorType,
            self.dateMatchTypeCriteria,
            self.daysLimitCriteria,
            self.amountLimitTypeCriteria,
            self.amountLimitSignCriteria,
            self.amountCriteria,
            self.accountCategoryCriteria,
            self.acceptAction,
            self.monthShiftAction,
            self.removeAction,
            self.textAction,
            self.commentAction,
            self.tagAction,
            self.categoryIdAction,
            self.splitActions,
            self.flagAction];
}
...
- (NSDictionary *)propertyValueTransformers {
    return @{
        @"createdDate": [MNFBasicDateValueTransformer transformer],
...
- (NSDictionary<NSString *, MNFJsonAdapterSubclassedProperty *> *)subclassedProperties {
    return @{
        @"splitActions": [MNFJsonAdapterSubclassedProperty subclassedPropertyWithClass:[MNFTransactionSplitAction class]
                                                                                option:kMNFAdapterOptionNoOption]
    };
}
