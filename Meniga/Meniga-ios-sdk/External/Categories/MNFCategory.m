    ...
    __block MNFJob *job = [self
        apiRequestWithPath:path
                ...
                HTTPMethod:kMNFHTTPMethodGET
                   service:MNFServiceNameCategories
...
+ (MNFJob *)fetchUserCreatedCategoriesWithCulture:(NSString *)theCulture
                                       completion:(MNFMultipleCategoriesCompletionHandler)completion {
    __block MNFJob *job = [self
        p_fetchCategoriesWithExcludedUserCreatedCategories:nil
                                                   culture:theCulture
                                                      skip:nil
                                                      take:nil
                                                completion:^(NSArray *categories, NSError *error) {
                                                    if (categories != nil) {
                                                        categories = [self
                                                            p_filterCategoryIsSystemCategory:[NSNumber
                                                                                                 numberWithBool:NO]
...
+ (MNFJob *)fetchSystemCategoriesWithCulture:(NSString *)theCulture
                                  completion:(MNFMultipleCategoriesCompletionHandler)completion {
    __block MNFJob *job = [self
        p_fetchCategoriesWithExcludedUserCreatedCategories:nil
                                                   culture:theCulture
...
+ (MNFJob *)fetchCategoriesWithCulture:(NSString *)theCulture
                            completion:(MNFMultipleCategoriesCompletionHandler)completion {
    __block MNFJob *job =
        [self p_fetchCategoriesWithExcludedUserCreatedCategories:[NSNumber numberWithBool:NO]
                                                         culture:theCulture
                                                            skip:nil
                                                            take:nil
                                                      completion:^(NSArray *categories, NSError *error) {
...
+ (MNFJob *)fetchCategoryTreeWithCulture:(NSString *)theCulture
                              completion:(MNFMultipleCategoriesCompletionHandler)completion {
    __block MNFJob *job =
        [self p_fetchCategoriesWithExcludedUserCreatedCategories:nil
                                                         culture:theCulture
                                                            skip:nil
                                                            take:nil
                                                      completion:^(NSArray *categories, NSError *error) {
...
+ (MNFJob *)p_fetchCategoriesWithExcludedUserCreatedCategories:(NSNumber *)excludeUserCreatedCategories
                                                       culture:(NSString *)theCulture
                                                          skip:(NSNumber *)theNumToSkip
                                                          take:(NSNumber *)theLimit
                                                    completion:
                                                        (MNFMultipleCategoriesCompletionHandler _Nullable)completion {
    NSString *path = [NSString stringWithFormat:@"%@", kMNFApiPathCategories];

    NSMutableDictionary *pathQuery = [NSMutableDictionary dictionary];

    NSString *isPublicString =
        [[MNFNumberToBoolValueTransformer transformer] reverseTransformedValue:excludeUserCreatedCategories];
                        ...
                        if ([response.result isKindOfClass:[NSArray class]]) {
                            ...
                            [MNFObject
                                executeOnMainThreadWithCompletion:completion
                                                   withParameters:nil
                                                              and:[MNFErrorUtils
                                                                      errorForUnexpectedDataOfType:[response.result
                                                                                                       class]
...
+ (MNFJob *)createUserParentCategoryWithName:(NSString *)theName
                              isFixedExpense:(NSNumber *)isFixedExpense
                                categoryType:(MNFCategoryTypeId)theCategoryType
                                  completion:(MNFCategoryCompletionHandler)completion {
    [completion copy];

    return [self p_createUserCategoryWithName:theName
...
+ (MNFJob *)createUserChildCategoryWithName:(NSString *)theName
    ...
    return [self p_createUserCategoryWithName:theName
                               isFixedExpense:isFixedExpense
                                 categoryType:theParentCategoryType
                             parentCategoryId:theParentCategoryId
                                   completion:completion];
}

#pragma mark - Private create user category

+ (MNFJob *)p_createUserCategoryWithName:(NSString *)theName
                          isFixedExpense:(NSNumber *)isFixedExpense
                            categoryType:(MNFCategoryTypeId)theCategoryType
                        parentCategoryId:(NSNumber *)theParentCategoryId
                              completion:(MNFCategoryCompletionHandler)completion {
        ...
        [MNFObject
            executeOnMainThreadWithCompletion:completion
                               withParameters:nil
                                          and:[MNFErrorUtils
                                                  errorWithCode:-223
                                                        message:@"A null parameter was sent to a parameter that must "
                                                                @"not be non null in create a user category."]];

        MNFJob *job =
            [MNFJob jobWithError:[MNFErrorUtils errorWithCode:-223
                                                      message:@"A null parameter was sent to a parameter that must not "
                                                              @"be non null in create a user category."]];
        return job;
    }
...
- (MNFJob *)saveWithCompletion:(MNFErrorOnlyCompletionHandler)completion {
    if ([self.isSystemCategory boolValue] == YES) {
        [MNFObject executeOnMainThreadWithCompletion:completion
                                       withParameter:[MNFErrorUtils errorWithCode:-199
                                                                          message:@"Cannot save a system category."]];
    ...
    NSMutableDictionary *mutableCategoryDict = [NSMutableDictionary
        dictionaryWithDictionary:[MNFJsonAdapter JSONDictFromObject:self option:kMNFAdapterOptionNoOption error:nil]];
...
- (MNFJob *)deleteCategoryWithConnectedRules:(NSNumber *)deleteConnectedRules
             moveTransactionsToNewCategoryId:(NSNumber *)theNewCategoryId
    ...
    NSString *deleteConnectedRulesBoolean =
        [[MNFNumberToBoolValueTransformer transformer] reverseTransformedValue:deleteConnectedRules];
...
- (void)p_populateChildArrayWithCategories:(NSArray *)theCategories {
    ...
    for (MNFCategory *category in theCategories) {
        ...
        if (category.parentCategoryId != nil) {
            if ([self.identifier isEqualToNumber:category.identifier] == NO &&
                [self.identifier isEqualToNumber:category.parentCategoryId] == YES) {
...
+ (NSArray<MNFCategory *> *)p_filterCategoryIsSystemCategory:(NSNumber *)isSystemCategory
                                               forCategories:(NSArray<MNFCategory *> *)theCategories {
...
- (NSString *)description {
    return [NSString
        stringWithFormat:@"Category %@ identifier: %@, name: %@, otherCategoryName: %@, parentCategoryId: %@, "
                         @"isSystemCategory: %@, isFixedExpenses: %@, categoryType: %d, categoryRank: %@, "
                         @"budgetGenerationType: %@, children: %@, categoryContextId: %@, orderId: %@, displayData: %@",
                         [super description],
                         self.identifier,
                         self.name,
                         self.otherCategoryName,
                         self.parentCategoryId,
                         self.isSystemCategory,
                         self.isFixedExpenses,
                         (int)self.categoryTypeId,
                         self.categoryRank,
                         self.budgetGenerationType,
                         self.children,
                         self.categoryContextId,
                         self.orderId,
                         self.displayData];
}
