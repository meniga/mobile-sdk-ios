...
+(MNFJob *)fetchTermsAndConditionTypes:(MNFMultipleTermsAndConditionTypesCompletionHandler)completion {
    ...
    __block MNFJob *job = [self apiRequestWithPath: kMNFTermTypes pathQuery: nil jsonBody: nil HTTPMethod:kMNFHTTPMethodGET service: MNFServiceNameTerms completion:^(MNFResponse *response) {
        ...
        if (response.error == nil) {
            ...
            if ([response.result isKindOfClass: [NSArray class]]) {
                ...
                NSArray *termsAndConditionTypes = [MNFTermsAndConditionType initWithServerResults: response.result];
                [MNFObject executeOnMainThreadWithJob: job completion: completion parameter: termsAndConditionTypes error: nil];
    ...
    __block MNFJob *job = [self apiRequestWithPath:  path pathQuery: nil jsonBody: nil HTTPMethod: kMNFHTTPMethodGET service: MNFServiceNameTerms completion:^(MNFResponse *response) {
        ...
        if (response.error == nil) {
            if ([response.result isKindOfClass: [NSDictionary class]]) {
                ...
                MNFTermsAndConditionType *termsType = [MNFTermsAndConditionType initWithServerResult: response.result];
                
                [MNFObject executeOnMainThreadWithJob: job completion: completion parameter: termsType error: nil];
...
-(NSDictionary *)jsonKeysMapToProperties {
    ...
    return @{
             @"identifier" : @"id",
              @"termsAndConditionTypeDescription" : @"description"
             };
